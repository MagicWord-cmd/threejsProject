...
function init(closured) {
    ...
    var runnerTexture = new THREE.TextureLoader().load('run.png');
    let annie = new TextureAnimator(runnerTexture, 10, 1, 10, 75); // texture, #horiz, #vert, #total, duration.
    var runnerMaterial = new THREE.MeshBasicMaterial({ map: runnerTexture, side: THREE.DoubleSide });
    var runnerGeometry = new THREE.PlaneGeometry(10, 10, 1, 1);
    var runner = new THREE.Mesh(runnerGeometry, runnerMaterial);
    runner.position.set(-10, 2.5, 0);
    scene.add(runner);

    var explosionTexture = new THREE.TextureLoader().load('/textures/sprites/fireworks/T_SS(5x4)_fireworks004.jpg');
    let boomer = new TextureAnimator(explosionTexture, 5, 4, 20, 50); // texture, #horiz, #vert, #total, duration.
