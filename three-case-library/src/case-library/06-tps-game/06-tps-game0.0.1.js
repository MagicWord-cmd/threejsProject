...
let mixer, character, actions, animations, activeAction, idleAction, walkAction, runAction, spurtAction;
...
function init() {
	...
	characterLoader.load(
		...
		gltf => {	//!所有的匿名函数都可以简写成箭头函数
			...
			character.children[0].children[0].material.map.encoding = THREE.sRGBEncoding;
			character.children[0].children[0].material.metalnessMap.encoding = THREE.LinearEncoding;
			character.children[0].children[0].material.roughnessMap.encoding = THREE.LinearEncoding;
			character.children[0].children[0].material.normalMap.encoding = THREE.LinearEncoding;
			character.children[0].children[0].material.normalScale = new THREE.Vector2(1, -1);    //!法线强度	
			...
			spurtAction = mixer.clipAction(animations[0]);
			console.log('mixer', mixer);
			actions = [idleAction, walkAction, runAction, spurtAction];
...
function animate() {
	...
	// if (mixer) {
	// // !使用if判断是解决异步调用时间差的另一种方式，但窗口会黑闪数次，体验不好
	// // !由于gltf异步加载需要时间，
	// // !这里把animate()函数放到加载完成后执行，
	// // !避免性能差的设备加载过慢而错过碰撞检测而掉入深渊
	// // !同时防止变量unfinded的错误而使程序中断
