...
/*    sprite要用的的引入    */
import { timerLocal, spritesheetUV, pointUV, vec2, texture, PointsNodeMaterial } from 'three/nodes';
...
let lightProbe, effect, cubeCamera;
...
function updatePlayer(deltaTime) {
	...
	if (!playerCollider.onFloor) {
		...
		playerCollider.velocity.y -= 3 * GRAVITY * deltaTime;
...
// 可由速度公式和条件v=0得到
// (6) 如果H>0,h>0，则H+h>0
let firecrackerVelocityY = Math.random() * 0.25 + 5.25;
let firecrackerDuration = firecrackerVelocityY / GRAVITY;	//重力加速度公式计算
let explosionePositionY = Math.pow(firecrackerVelocityY, 2) / (GRAVITY * 2);
...
firecrackerSprite.scale.set(0.4, 1.6, 0.4);
firecrackerSprite.position.set(Math.random(), 0, Math.random() - 100);
...
async function FireworksSystem() {
	...
	// 烟花播放帧数率
	const sheetFPS = Math.random() * 5 + 15;
	...
	await new Promise((resolve, reject) => {
		setTimeout(function () {
			...
			fireworksParticle.position.copy(explosionePosition);
